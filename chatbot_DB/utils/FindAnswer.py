class FindAnswer:
    ...
    def search(self, intent_name, ner_tags=None, ner_name=None):
        ...
        if ner_tags == 'B_QUES':
            ...
            sql = self._search_tel_query(dept_name['dept'])
            tel_num = self.db.select_one(sql)

            return answer['answer'], tel_num['tel_num'], dept_name['dept']
        elif ner_tags == 'B_DEPT':
            sql = self._search_tel_query(ner_name)
            tel_num = self.db.select_one(sql)

            return answer['answer'], tel_num['tel_num']
        else:
...
if __name__ == "__main__":
    ...
    ner_predict = [('안녕', None)]
    answer = answer_t.search('인사', ner_predict[0][1], ner_predict[0][0])

    answer_f = answer_t.tag_to_word(ner_predict, answer)
